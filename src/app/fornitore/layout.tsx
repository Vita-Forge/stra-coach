...
export default function FornitoreLayout({ children }: { children: ReactNode }) {
    const { user, isLoaded } = useAuth();
    const router = useRouter();

    useEffect(() => {
        if (!isLoaded) return;
        if (!user || user.role !== "fornitore") router.push("/sign-in");
    }, [user, isLoaded, router]);

    if (!user || user.role !== "fornitore") return null;
            ...
            <aside style={{ width: "200px", flexShrink: 0, display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)", background: "var(--iron-900)" }}>
                <div style={{ padding: "1.25rem", borderBottom: "1px solid var(--border)" }}>
                    <div style={{ fontFamily: "var(--font-display)", fontWeight: 900, fontSize: "1.1rem" }}>
                        STRA<span style={{ color: "var(--red-600)" }}>.</span>SUPPLY
                    </div>
                    <div style={{ fontSize: "0.6rem", color: "var(--text-muted)", marginTop: "0.2rem", textTransform: "uppercase", letterSpacing: "0.1em" }}>Partner Fornitore</div>
                </div>
                <nav style={{ flex: 1, padding: "0.5rem 0" }}>
                    <a href="/fornitore" className="nav-item active">Ordini & Catalogo</a>
                </nav>
                <div style={{ padding: "1rem", borderTop: "1px solid var(--border)" }}>
                    <div style={{ fontSize: "0.72rem", fontWeight: 600 }}>{user.name}</div>
                    <div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{user.specialty}</div>
